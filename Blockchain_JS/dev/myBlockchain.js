...
Blockchain.prototype.createNewBlock = function(nonce, preBlockHash, hash)
{
    ...
    const newBlock = {
        ...
        preBlockHash: preBlockHash           //이전블록의 해시값
    }
...
Blockchain.prototype.hashBlock = function(preBlockHash, curBlockData, nonce)
{
    ...
    const dataString = preBlockHash + nonce.toString() + JSON.stringify(curBlockData);
...
Blockchain.prototype.chainIsValid = function(blockchain) {
    ...
    for ( var i = 1; i < blockchain.length; i++){
        ...
        // 해시값이 일치하는지 확인
        if (currentBlock['preBlockHash'] !== prevBlock['hash'])
        {
            validChain = false;
        };
    };
    ...
    const correctPreviousBlockHash = genesisBlock['preBlockHash'] ==='0';
